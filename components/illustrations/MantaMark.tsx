/**
 * Značka Manta — origami rejnok před sluncem/bowlingovou koulí.
 * Geometrie odpovídá originálnímu logu dodanému klientem (ploché fasety,
 * bez gradientů). Wordmark „manta“ se sází živým fontem (Poppins 700),
 * ne obtaženými křivkami — viz components/layout/Logo.tsx.
 */
export function MantaMark({ className }: { className?: string }) {
  return (
    <svg viewBox="29 13 161 156" className={className} aria-hidden>
      {/* Slunce / bowlingová koule */}
      <circle cx="131" cy="63" r="46" fill="#F9D086" />
      <circle cx="133" cy="49" r="5" fill="#FFFFFF" />
      <circle cx="153" cy="46" r="4" fill="#FFFFFF" />
      <circle cx="147" cy="60" r="5" fill="#FFFFFF" />

      {/* Origami rejnok */}
      <polygon fill="#89C8E1" points="36,72 102,67 147,97 104,130 80,165 97,126 73,99" />
      <polygon fill="#55A3D0" points="129,79 132,108 108,133 152,134 186,148 174,119 149,75" />
      <polygon fill="#B8D8E1" points="33,73 87,66 73,100" />
      <polygon fill="#B8D8E1" points="123,60 103,67 98,130 128,78" />
    </svg>
  );
}
